...
class TestUser:
    ...
    def test_user_sign_up_existing_user_error_is_returned(
        self, register_user_data, register_new_user
    ):
        response = UserApi.register_new_user(register_user_data)
        error_message = response.json()["message"]
        assert (
            response.status_code == ResponseStatus.BAD_REQUEST
            and error_message == ErrorMessages.USER_EXISTS_ERROR_MESSAGE
        )

    def test_user_login_user_is_successfully_logged_in(
        self, login_user_data, register_new_user
    ):
